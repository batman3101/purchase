  ...
  try {
        ...
        tableRows += `
          <tr>
              ...
              <td>
                  ...
                  ${cancelButton}
                  <button class="btn btn-danger btn-sm" onclick="deletePO('${escapeHtml(row[0])}')">삭제</button>
              </td>
          </tr>
        `;
      });
    ...
    return `
      <div class="d-flex justify-between align-center mb-2">
          <h3 style="margin:0;">발주(PO) 관리</h3>
          <button type="button" class="btn btn-primary" onclick="showAddPOModal()">새 PO 생성</button>
      </div>
      <table class="styled-table">
          <thead>
              <tr>
    ...
    Logger.log(e);
    return `<p style="color: red;">'PO 관리' 로드 실패: ${e.message}</p>`;
  }
}

/**
 * 거래명세서 페이지 HTML 생성
 * [수정] 파일 업로드 폼에 메타데이터 필드 추가
 */
function getStatementPage() {
  try {
    ...
    if (lastRow > 1) {
      // StatementID, IssueDate, SupplierID, PO_IDs, Amount, FileLink
      ...
      <div class="card mb-2">
          ...
          <form id="upload-form-statement">
              ...
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                  ...
                  <div class="form-group">
                      <label for="statement-amount">금액 (VND)</label>
                      <input type="number" id="statement-amount" name="amount" class="form-control" placeholder="0" required>
                  </div>
              </div>
              <div class="form-group">
                  <label for="statement-pos">관련 PO 번호 (선택)</label>
                  <input type="text" id="statement-pos" name="poIds" class="form-control" placeholder="예: PO-1001, PO-1002">
              </div>
